...
then
	...
	faSize $REF_FASTA -detailed > $REF_DIR/$REF_NAME.size
fi
...
do
	...
	faSize $REF_DIR/chr_seq/$chr_name.fa -detailed > $REF_DIR/chr_seq/$chr_name.size
done
...
if [ ! -d "$annot_dir/$REF_NAME.chain.net" ]; then
...
do
	...
	if [ ! -e $annot_dir/codex/$chr_name.codex ]; then
