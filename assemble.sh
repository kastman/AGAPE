...
then
	...
	ln -s $fname1 $lname1
	abyss-pe aligner=map k=$K_MER name=$seq_name se='seq1.fastq'
elif [ $mode == 2 ]
then
	...
	ln -s $fname3 $lname3
	ln -s $fname4 $lname4
	abyss-pe aligner=map k=$K_MER name=$seq_name lib='pe1' pe1='seq1.fastq seq2.fastq' se='seq_se1.fastq seq_se2.fastq'
...
then
	...
	ln -s $fname1 $lname1
	ln -s $fname2 $lname2
	abyss-pe aligner=map k=$K_MER name=$seq_name lib='pe1' pe1='seq1.fastq seq2.fastq'
else
